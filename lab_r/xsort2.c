...
char restype[32];              /* Resource type space */
...
KeySym ks = XLookupKeysym((XKeyEvent*) ev, 0);
	switch(ks)
	{
		case XK_o: if(ev->xkey.state & ControlMask)
			return(1);
		break;
		default: break;	
	}
...
int dispatch() {
int done = 0;                  /* event loop continue flag */
...
while(done == 0) {                /* event loop */
  ...
  switch(event.type) {            /* check event type */
     case Expose: expo(&event);   /* redraw all fragments */
                  break;
      case ButtonPress: /* begin new rubber or rubbout fragment */
                        if(rubin(&event) != Button1) /* new fragment */
                          rubout(&event);            /* del fragment */
                        break;
      case MotionNotify: rerub(&event);    /* redraw rubber fragment */
                         break;
      case ButtonRelease: /* end rubber fragment */
                          if(fix(&event) > 0)    /* fix new fragment */
                            rextra();        /* reset extra fragment */
                          refrag();          /* redraw all fragments */
...
pass(dpy, win, gc); /* pass graphics to xsort1 */
pass1(dpy, win, gc); /* pass graphics to xsort */
